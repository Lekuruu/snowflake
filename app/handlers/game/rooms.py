...
async def on_room_to_room_min_time(client: Penguin, data: dict):
...
async def on_room_to_room_complete(client: Penguin, data: dict):
    if not client.in_game:
        return
...
async def on_room_to_room_screen_closed(client: Penguin, data: dict):
...
async def on_room_to_room_member_revive_tip(client: Penguin, data: dict):
    # client.send_tip(Phase.MEMBER_CARD)
    ...

@session.framework.register('roomToRoomMemberBuyCardsTip')
async def on_room_to_room_member_buy_cards_tip(client: Penguin, data: dict):

...
async def on_animation_done(client: Penguin, object_id: int, handle_id: int):
...
async def on_sound_done(client: Penguin, object_id: int, handle_id: int):
    """Sent by the client after a sound is done playing"""
    client.game.callbacks.action_done(handle_id, object_id)

@session.events.register('/intro_anim_done')
async def on_intro_done(client: Penguin):
